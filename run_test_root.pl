...
This is a Q-DDS Publisher and Subscriber both single run program. When the it is ready, it will send / receive data using Q-DDS.
...
my $opts = "4000 -DCPSBit 0 -DCPSConfigFile rtps_disc.ini $reliable $wait_for_acks";
my $pub_opts = $opts;
my $sub_opts = $opts;
if ($debug ne '0') {
...
my $SubscriberResult = $SUB->WaitKill(3600);
